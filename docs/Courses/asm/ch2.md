...
其中，图片下半部分的*浮点寄存器*（左）和*向量寄存器*（右）为 x87 新增的。
...
如下表格更直观的比较了 Intel IA-32 和 64-bit Mode 二者的寄存器差异，其中深色部分表示存在不同：
...
| `D`      | Data              | Extended space for accumulator<br>常存储溢出的部分结果数据，例如乘法的高位结果以及除法被除数的高位部分                                                             |
| `BP`     | Base Pointer      | Pointer to address of current stack frame<br>尤其是函数调用的时候，函数参数结束、局部变量开始的地方                                                         |
| `SP`     | Stack Pointer     | Pointer to address of last bytes PUSHed to memory<br>指向栈顶的偏移                                                                     |
...
64-bit mode 下，memory 的管理方式从*分段管理*变为*分页管理*，`DS`, `ES`, `SS` 被忽视（置 0），即便我们尝试对其修改也可能不起作用：
...
!!! warning "保护模式下会对段内偏移作检测，即要求 offset 小于该段的 limit，而实模式不会"

### Protected Mode Memory Addressing

在保护模式下，段寄存器不再单纯存储段地址，而是存储指向 **Descriptor Table** 中某一描述符的 **Selector**。

例如对于指令 `MOV [BX], AX`，它的访存流程如下：

![[ch2_17.png]]

**Descriptor** 描述了一个内存段的地址，长度以及访问权限，具体类型包括 `Code`, `Stack`, `Data`, `Gate`, `Task-State Segment`, `Local-Decriptor Table` 等。它的结构随着架构的升级变更如下：

![[ch2_14.png]]

!!! success "文件描述符 `Code` 和 `Gate` 的区别"
	代码段描述符指向内存中存放指令的一块连续区域；门描述符则指向一个函数的入口点，通常用于中断/陷阱等涉及特权级切换的过程调用。
...
- **GDT:** 包含所有程序可用的描述符，是必须的
	- 全局描述符表的第一个 entry 称为 null descriptor，保留给不使用的段寄存器
	- 将 null descriptor 加载到段寄存器中不会产生异常，但是使用它去访问内存则会出现 general-protection exception
- **LDT:** 包含一个程序使用的描述符，是可选的
- **IDT:** 只包含门描述符，且是必须的

!!! note "描述符表最大 64KB，一个描述符长 8B，这意味着每个表最多有 8192 个 entries"
...
与计组中学到的分页稍有区别的是，现代 CPU 通常会采用 Multi-level Paging，而不是只通过一个 Page Table 就能转换虚拟地址和物理地址。这是因为 Single-level 往往占用太多内存。
...
$$
\text{Page Attribute = PDE Attribute } \land \text{ PTE Attribute}
$$
...
- **Control Register 4:** Pentium 开始新增的控制寄存器，拓展了原有的分页架构控制
	...
	- PAE(bit 5) 是否启用物理地址拓展。开启后允许物理地址空间超过 4GB
		...
		- 此时 PTE 的大小由 4B 变为 8B，因此从二级分页 10-10-12 变为三级分页 2-9-9-12
...
假定我们要对一个 Page Table 进行访问，由于我们只能通过虚拟内存来与 Memory 交互，我们该如何得到 Page Table 中一个 entry 的虚拟内存？
...
=== "访问虚拟地址 `0xe4321000` 经过的 PTE"
	![[ch2_30.png]]
=== "访问虚拟地址 `0xc0300000` 经过的 PDE"
	![[ch2_36.png]]
...
**Addressing Modes** 是用来指定操作数地址的技术，包括数据寻址、代码寻址和栈寻址。
...
- **Direct Data Addressing (Disp)** 直接数据寻址
...
- **Register Indirect Addressing (Base / Index)** 寄存器间接寻址
	- 只可使用 `BX`, `BP`, `SI`, `DI`
	- 80386 开始，所有 32-bit 寄存器都可使用
- **Base-Plus-Index Addressing (Base + Index)** 基址加变址寻址
	- `base` 可选 `BX` 和 `BP`，通常存储数组的起始位置；`index` 可选 `SI` 和 `DI`，通常存储数组中元素的相对偏移；`scale` 可选 1,2,4,8
	- 从 80386（32-bit）开始，所有 32-bit 寄存器都可以作为 `base` 和 `index`
	- 当方括号中选用寄存器 `BP`/`EBP`/`ESP` 作为 `base` 时，则缺省段址默认为 `SS`；否则，均为 `DS`
- **Register Relative Addressing (Base/Index + Disp)** 寄存器相对寻址
- **Base Relative-Plus-Index Addressing (Base + Index + Disp)** 基址相对加变址寻址
- **Scaled-Index Addressing (Base+Scale×Index+Disp)** 比例变址索引
...
??? question "Determine the addressing type"
	- `MOV EAX, [3000H] ; a1 00 30 00 00`
		- Direct Addressing，操作码 `A1` 专门用于“将内存地址的内容移动到 `EAX`”
	- `MOV EBX, [3000H] ; 8b 1d 00 30 00 00`
		- Displacement Addressing，它使用通用操作码 `8B`，后面必须跟一个 ModR/M 字节（`1D`）
	- `ADD EAX, [3000H] ; 03 05 00 30 00 00`
		- Displacement Addressing，`ADD` 并没有像 `MOV` 一样给 `EAX` 提供特殊操作码
	- `ADD EAX, 3000H ; 05 00 30 00 00`
		- 属于立即数寻址了，与上面几个 Memory Address 不是同一类
		- 不过提一嘴，操作码 `05` 专门用于“累加器加 32 位立即数”
...
通过检查 canonical-address form，我们可以防止软件利用不使用的高位地址去做其它事情。
...
除此之外，我们还可以读取内存中的 Effective Address 作为偏移或目标地址，这被称为 *Indirect Program Memory*，例如 `JMP NEAR PTR [DI+2]`。

该寻址方式可被用来书写跳转表，例如如下代码：

```asm
data segment
TABLE DW LOC0, LOC1, LOC2, LOC3
data ends
...
- `PUSHA` push all
	...
	- `SP` 压入的是原始值
	- 操作数大小为 2 Bytes，共 16B
- `PUSHAD` push all double
	- 实际上是入栈所有 32-bit 寄存器
	- 操作数大小为 4 Bytes，共 32B
- `PUSHF` push flags
	- 将 FLAG 入栈
