...
对于一个长度为 n 的操作序列，在最坏情况所用的总时间为 $T(n)$，那么其每个操作摊还时间可以简单认定为：
...
其中， $\phi(D_i)$ 称为势能函数(Potential Function)，且一定有 $\phi(D_n) \ge \phi(D_0)$
...
那么每一步操作的摊还消耗即可定义为 $c_i+ \phi(D_i) - \phi(D_{i-1})$ ，势能分析法中的摊还思想体现在如果该操作消耗 $c_i$ 很大，可以通过定义的势能函数让 $\phi(D_i) - \phi(D_{i-1})$ 的结果尽可能小。由于数据结构最终势能是上升的，代价小的操作可以分摊代价大的操作的势能。
...
此时就需要用到摊还分析，由于 MultiPop 是时间消耗大的操作，我们需要让该步操作势能差值尽可能小。由于无论是 Push 操作还是 Pop 操作，它们都改变了栈中元素的个数，我们可以想到以栈内元素个数作为势能函数 $\phi(D_i)$ 的定义
...
对 Splay Tree 操作消耗较大的情况有访问一个叶子节点导致调整很多次树的结构。为了分摊该操作的代价，需要让其势能差值尽可能小，可以想到采用量化衡量树的平衡情况的函数：

$$\begin{gather}
D_i = The\ Root\ of\ the\ Resulting\ Tree \\
\phi(D_i) = \sum_{i\in D_i}\log S(i)\ 其中 S(i) 表示以i为根的子树大小
\end{gather}
$$

!!! info "log S(i) 可以约等于该子树的高度"
